...
describe('detectAndDelegate', () => {
  ...
  it('delegates when content mentions a valid target', async () => {
    ...
    await detectAndDelegate('issue-1', stratId, 'Please hand this to the **Tech Lead** for implementation.');
  ...
  it('does not delegate to agents outside the org chart', async () => {
    ...
    await detectAndDelegate('issue-2', stratId, 'Send to the deployer for release.');
  ...
  it('does not delegate when no agent names found', async () => {
    const stratId = AGENTS.strategist;
    await detectAndDelegate('issue-3', stratId, 'This looks good, no further action needed.');
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it('strips markdown formatting when detecting agents', async () => {
    const stratId = AGENTS.strategist;
    await detectAndDelegate('issue-4', stratId, 'Route this to **Tech Lead** for review.');
  ...
  it('propagates remote flag when delegating to local builder', async () => {
    ...
    await detectAndDelegate('issue-5', techId, 'Send to the local builder for implementation.');
  ...
  it('keeps remote flag for intermediate hops', async () => {
    ...
    await detectAndDelegate('issue-6', stratId, 'Send to the tech lead for breakdown.');
  ...
  it('reroutes "local builder" mention from Strategist to Tech Lead', async () => {
    const stratId = AGENTS.strategist;
    await detectAndDelegate('issue-7', stratId, 'Send to the local builder for coding.');
